...
- Input schema accepts 2–4 candidate sites. Output schema always includes: ranking, per-site itemized cost breakdown (CapEx/OpEx, fixed/variable), lifetime cost per kW (whole-life cost ÷ capacity), build cost per kW (capex ÷ capacity), NPV/payback, low/base/high range, a "this ranking flips if…" sentence, and a plain-English recommendation paragraph.
